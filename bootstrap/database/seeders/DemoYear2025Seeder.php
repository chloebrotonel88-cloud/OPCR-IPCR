        ...
        foreach ($plan as $title => $emails) {
            ...
            foreach ($output->indicators as $index => $line) {
                ...
                for ($slot = 0; $slot < $take; $slot++) {
                    ...
                    $child    = $assignments->assignIndicator($line, $assignee, $president);

                    // The OPCR line is year-wide; the IPCR it lands in is not.
                    $child->update([
                        'target_date'      => $line->target_date,
                        'rating_period_id' => $child->output->form->rating_period_id,
                    ]);
                }
            }
        }
