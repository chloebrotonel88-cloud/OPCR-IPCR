...
class BulkCascadeTest extends PmsTestCase
{
    ...
    public function test_every_line_reaches_every_person_in_one_call(): void
    {
        ...
        foreach ([$head, $faculty] as $person) {
            ...
            $children = PcrIndicator::whereHas('output', fn ($q) => $q->where('form_id', $form->id))->get();

            $this->assertCount(3, $children);
            $this->assertTrue($children->every(fn ($line) => (int) $line->rating_period_id === $period->id));
        }
    }
    ...
    public function test_only_the_named_lines_are_handed_out(): void
    {
        ...
        $this->assertDatabaseHas('pcr_indicators', ['parent_indicator_id' => $lines[0]->id]);
        $this->assertDatabaseMissing('pcr_indicators', ['parent_indicator_id' => $lines[1]->id]);
    }
