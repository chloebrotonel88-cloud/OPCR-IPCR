...
import { ADJECTIVAL_COLORS, DELAY_META, SECTION_LABELS } from "~/utils/constants";
...
export default function IpcrSheet({
    ...
    SECTIONS.forEach((section) => {
        ...
        outputs.forEach((output, outputIndex) => {
            ...
            const fromCollege = Boolean(output.parent_output_id);
            ...
            const outputCell = (
                <td rowSpan={span} className="pms-sheet-output" data-output-id={output.id}>
                    <div className="pms-sheet-number">
                        <span>{number}.</span>
                        {fromCollege ? (
                            <Tooltip title="Named by the college OPCR — add your success indicators under it">
                                <span className="pms-sheet-locked">{output.title}</span>
                            </Tooltip>
                        ) : (
                            <SheetCell
                                value={output.title}
                    ...
                    {(canEdit || canAssignHeadings) && (
                        <Space size={2} className="pms-sheet-rowtools">
                            ...
                            {canEdit && hasOpcrTargets && (
                                <Tooltip title="Add a line tied to an office target">
                                    <Button
                                        size="small"
                                        type="text"
                                        icon={<LinkOutlined />}
                                        onClick={() => onAddIndicator?.(output)}
                                    />
                                </Tooltip>
                            )}
                        ...
                        <td>
                            ...
                            {line.parent && (
                                <Tooltip title={toPlainText(line.parent.description)}>
                                    <Tag style={{ marginTop: 4, fontSize: 10 }} color="geekblue">
                                        Office target
                                    </Tag>
                                </Tooltip>
                            )}
            ...
            >
                <Typography.Paragraph type="secondary">
                    {assigning?.kind === "output"
                        ...
                        : "Each person gets this line in their own IPCR, seeded with its wording for them to reword. Anything you do not hand out stays yours to deliver."}
