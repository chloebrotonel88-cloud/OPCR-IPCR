...
import { ADJECTIVAL_COLORS, SECTION_LABELS, STATUS_META } from "~/utils/constants";
...
export default function FormEditorPage({ formId = null }) {
    ...
    const [indicatorFor, setIndicatorFor] = useState(null);
    ...
    const { data: opcrOutputs = [] } = useQuery({
        queryKey: ["opcr-outputs", id],
        queryFn: () => api.get(`pcr-forms/${id}/opcr-outputs`).then((r) => r.data),
        enabled: Boolean(id),
    });
    ...
    const isOwner = useMemo(() => {
        ...
        // The president owns the college OPCR wherever they sit; heads and VPs
        // own one written for their own office.
        if (user.role === "president") return true;
        return form.org_unit_id === user.org_unit_id && can("program_head", "vp");
    }, [form, user, can]);
                ...
                <Col xs={24} lg={24}>
                    ...
                    >
                        {isOpcr ? (
                            ...
                            <Empty description="No outputs yet. Add an MFO/PPA, then list the success indicators under it." />
                        ) : (
                            <IpcrSheet
                                form={form}
                                periodId={activePeriodId}
                                ...
                                hasOpcrTargets={opcrTargets.length > 0}
                                summary={summary}
                                onAddOutput={(section) => {
                                    outputForm.setFieldsValue({ section });
                                    setOutputModal(true);
                                }}
                                onAddIndicator={setIndicatorFor}
                            />
                        )}
                    </Card>
                </Col>
...
{summary && (
                ...
                >
                    ...
                    <Form.Item noStyle shouldUpdate={(a, b) => a.section !== b.section}>
                        {({ getFieldValue }) => {
                            ...
                            const fromCollege =
                                !isOpcr && section !== "support" && opcrOutputs.length > 0;

                            return fromCollege ? (
                                <Form.Item
                                    name="parent_output_id"
                                    label="College MFO/PPA"
                                    rules={[
                                        { required: true, message: "Pick the MFO/PPA this answers to." },
                                    ]}
                                    extra="Your core commitments sit under something the college named in its OPCR."
                                >
                                    <Select
                                        showSearch
                                        optionFilterProp="label"
                                        placeholder="Select an MFO/PPA"
                                        options={opcrOutputs
                                ...
                                <Form.Item
                                    name="title"
                                    label="Output"
                                    rules={[{ required: true, message: "Name the output." }]}
                                    extra={
                                        isOpcr
                                ...
                                >
                                    <Input placeholder={isOpcr ? "Research" : "Submission of DTR"} />
                                </Form.Item>
                            );
                        }}
                    </Form.Item>
            ...
            <Modal
                title={`Add success indicator under “${indicatorFor?.title ?? ""}”`}
                open={Boolean(indicatorFor)}
                onCancel={() => setIndicatorFor(null)}
                onOk={() => indicatorForm.submit()}
                confirmLoading={addIndicator.isPending}
            >
                <Form
                    form={indicatorForm}
                    layout="vertical"
                    requiredMark={false}
                    onFinish={(v) => addIndicator.mutate(v)}
                >
                    <Form.Item
                        name="description"
                        label="Success indicator"
                        rules={[{ required: true, message: "Describe the target and how it is measured." }]}
                        extra="Target + measure + timeframe, e.g. “Publish 25 peer-reviewed articles from January to December 2026.”"
                    >
                        <RichText rows={4} placeholder="Publish 25 peer-reviewed articles from January to December 2026." />
                    </Form.Item>
                    {isOpcr && (
                        <>
                            <Form.Item name="allotted_budget" label="Allotted budget">
                                <InputNumber
                                    style={{ width: "100%" }}
                                    min={0}
                                    prefix="₱"
                            ...
                            <Form.Item name="accountable" label="Individual / accountable">
                                <Input placeholder="Ronnel D. Pinto / Dr. Demetrio" />
                            </Form.Item>
                        </>
                    )}
                    <Form.Item
                        name="target_date"
                        label="Target date"
                        extra="When this task should be accomplished. Progress is tracked against it."
                    >
                        <DatePicker style={{ width: "100%" }} format="MMM D, YYYY" />
                    </Form.Item>
                    {!isOpcr && opcrTargets.length > 0 && (
                        <Form.Item
                            name="parent_indicator_id"
                            label="Contributes to office target"
                            extra="Optional. The heading above already ties this to the college OPCR; name a specific target only if it helps."
                        >
                            <Select
                                allowClear
                                placeholder="Select an OPCR target"
                                optionFilterProp="label"
                                showSearch
                                options={Object.entries(
                                    opcrTargets.reduce((groups, t) => {
                                        const key = SECTION_LABELS[t.section] ?? t.section;
