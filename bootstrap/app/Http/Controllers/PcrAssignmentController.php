...
class PcrAssignmentController extends Controller
{
    ...
    public function storeForIndicator(Request $request, $id)
    {
        ...
        foreach ($this->assigneesFrom($data, $form) as $assignee) {
            $already = PcrIndicator::where('parent_indicator_id', $parent->id)
                ->whereHas('output.form', fn ($q) => $q->where('user_id', $assignee->id))
                ->exists();
    ...
    public function cascade(Request $request, $id)
    {
        ...
        foreach ($lines as $line) {
            foreach ($assignees as $assignee) {
                $already = PcrIndicator::where('parent_indicator_id', $line->id)
                    ->whereHas('output.form', fn ($q) => $q->where('user_id', $assignee->id)
                        ->where('rating_period_id', $periodId))
                    ->exists();
        ...
        foreach ($touched as $userId => $count) {
            Notification::send(
                ...
                'They are waiting in your IPCR as drafts, cascaded from ' . strtoupper($form->type) . '.',
                $form->id
            );
        }
    ...
    {
        $child  = PcrIndicator::with('output.form')->findOrFail($childId);
        $parent = $child->parent;
