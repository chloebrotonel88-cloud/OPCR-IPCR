...
class PcrIndicatorController extends Controller
{
    public function store(Request $request)
    {
        ...
        if ($parentId) {
            if ($error = $this->parentProblem($parentId, $output->form, $periodId, $id)) {
        ...
        $attributes = [
            ...
            'allotted_budget'   => $isOpcr ? ($data['allotted_budget'] ?? null) : null,
            'accountable'       => $isOpcr ? ($data['accountable'] ?? null) : null,
        ...
        if ($id) {
            ...
            return response()->json(['data' => 'updated', 'indicator' => $indicator]);
        }
    ...
    public function progress(Request $request, $id)
    {
        ...
        if ($indicator->children()->exists()) {
            return response()->json([
                'message' => 'This line is rolled up from the sub-tasks under it. Update those instead.',
            ], 409);
        }
     ...
     */
    private function parentProblem(int $parentId, $form, ?int $periodId, $selfId): ?string
    {
