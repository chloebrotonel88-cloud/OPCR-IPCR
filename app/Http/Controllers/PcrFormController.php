...
class PcrFormController extends Controller
{
    ...
    public function show(Request $request, $id)
    {
        ...
        $form->loadMissing([
            'outputs.indicators.children' => fn ($query) => $query
                ->select([
                    'id', 'parent_indicator_id', 'output_id', 'description',
        ...
        $form->outputs->loadCount('childOutputs');
    ...
    private function notifyTransition(PcrForm $form, string $to, ?string $note): void
    {
        ...
        if ($to === 'qa_rating') {
            Notification::sendMany(
                User::where('role', 'qa')->where('status', 'active')->pluck('id')->all(),
                'rating',
    ...
    {
        $form = PcrForm::with('outputs.indicators.children')->findOrFail($id);
    ...
    {
        return PcrForm::with('outputs.indicators')
            ->where('type', 'opcr')
    ...
    private function targetsFor(PcrForm $form)
    {
        ...
        return $opcr->outputs->flatMap(function ($output) use ($form) {
    ...
    private function scopeToOwner($query, User $user): void
    {
        ...
        $query->where(function ($q) use ($user, $opensOpcr) {
            ...
            if ($opensOpcr) {
                $q->orWhere('type', 'opcr');
            } elseif (in_array($user->role, ['program_head', 'vp'], true) && $user->org_unit_id) {
